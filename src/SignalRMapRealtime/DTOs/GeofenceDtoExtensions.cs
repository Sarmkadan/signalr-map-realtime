...
public static class GeofenceDtoExtensions
{
	...
	public static double? DistanceTo(this GeofenceDto dto, double latitude, double longitude)
	{
		...
		return dto.CenterLatitude is null || dto.CenterLongitude is null || dto.RadiusKm is null
			? null
			: GeoLocationExtensions.DistanceBetween(
	...
	{
		ArgumentNullException.ThrowIfNull(dto);
		return dto.Type == nameof(GeofenceType.Circle);
	}
	...
	{
		ArgumentNullException.ThrowIfNull(dto);
		return dto.Type == nameof(GeofenceType.Polygon);
	}
	...
	private static bool ContainsPolygon(string? polygonCoordinates, double latitude, double longitude)
	{
		...
		var points = ParsePolygonCoordinates(polygonCoordinates);
		if (points.Count < 3)
			return false;
	...
	static List<(double Latitude, double Longitude)> ParsePolygonCoordinates(string coordinates)
	{
		var points = new List<(double, double)>();
		foreach (var pair in coordinates.Split(';', StringSplitOptions.RemoveEmptyEntries))
		{
			var parts = pair.Split(',');
			if (parts.Length == 2
				&& double.TryParse(parts[0].Trim(), NumberStyles.Float, CultureInfo.InvariantCulture, out var lat)
